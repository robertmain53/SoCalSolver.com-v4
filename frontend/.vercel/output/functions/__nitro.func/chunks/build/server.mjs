...
const _routes = [
  ...
  },
  {
    name: "builder___en",
    path: "/en/builder",
    component: () => import('./builder-e9ar6Pz-.mjs')
  },
  {
    name: "builder___it",
    path: "/it/builder",
...
function getAlternateOgLocales(options, currentLanguage = options.getCurrentLanguage()) {
...
function localePath(ctx, route, locale = ctx.getLocale()) {
  ...
  try {
    ...
    return ((_a = localizedRoute == null ? void 0 : localizedRoute.redirectedFrom) == null ? void 0 : _a.fullPath) || localizedRoute.fullPath;
  } catch {
    return "";
  }
}
...
function createHeadContext(ctx, config, locale = ctx.getLocale(), locales = ctx.getLocales(), baseUrl = ctx.getBaseUrl(), routingOptions = ctx.getRoutingOptions()) {
  ...
  const canonicalQueries = typeof config.seo === "object" && ((_a = config.seo) == null ? void 0 : _a.canonicalQueries) || [];
  if (!baseUrl) {
...
function localeHead(ctx, { dir = true, lang = true, seo = true, key = "key" }) {
  return localeHead$1(createHeadContext(ctx, { dir, lang, seo, key }));
}
const localeCodes = [
  "en",
  "it",
...
function getRouteNameLocaleRegex(options, localesPattern = getLocalesPattern(options.localeCodes)) {
...
function createNameLocaleRegexMatcher(re) {
  return (val) => {
    var _a;
    return ((_a = val.match(re)) == null ? void 0 : _a[1]) ?? "";
  };
}
...
function getRouteName(route) {
  if (typeof route === "object") return normalizeRouteName(route == null ? void 0 : route.name);
  return normalizeRouteName(route);
}
function getRouteBaseName(route, separator) {
  return getRouteName(route).split(separator)[0];
}
function getLocalizedRouteName(routeName, locale, isDefault, separator = "___", defaultSuffix = "default") {
  return routeName + separator + locale;
}
function getRoutePathLocaleRegex(localeCodes2, localesPattern = getLocalesPattern(localeCodes2)) {
  return new RegExp(`^/${localesPattern}(?:/|$)`, "i");
}
function getLocalesPattern(localeCodes2) {
  return `(${localeCodes2.join("|")})`;
}
function createLocaleFromRouteGetter(options) {
  ...
  return (route) => {
    ...
    if (route == null ? void 0 : route.path) {
...
function getCompatRoutePath(route) {
...
function createI18nCookie() {
  const detect = (/* @__PURE__ */ useRuntimeConfig()).public.i18n.detectBrowserLanguage;
  const cookieKey = detect && detect.cookieKey || "i18n_redirected";
...
function detectBrowserLanguage(route, localeCookie, locale = "") {
  const _detect = (/* @__PURE__ */ useRuntimeConfig()).public.i18n.detectBrowserLanguage;
  if (!_detect) {
  ...
  if (!firstAccess) {
    return { locale: "", error: "first_access_only" };
  }
  {
...
function getHost() {
  return getRequestHost(useRequestEvent(), { xForwardedHost: true });
}
function getProtocol(nuxt) {
  return getRequestProtocol(useRequestEvent(nuxt)) + ":";
}
function createDomainFromLocaleGetter(nuxt) {
  const host = getHost();
  const { domainLocales } = (/* @__PURE__ */ useRuntimeConfig()).public.i18n;
  return (locale) => {
    var _a, _b;
    const lang = normalizedLocales.find((x) => x.code === locale);
    const domain = ((_a = domainLocales == null ? void 0 : domainLocales[locale]) == null ? void 0 : _a.domain) || (lang == null ? void 0 : lang.domain) || ((_b = lang == null ? void 0 : lang.domains) == null ? void 0 : _b.find((v) => v === host));
    if (!domain) {
...
async function loadVueI18nOptions(vueI18nConfigs2, nuxt = nuxtMock) {
...
async function loadLocale(locale, localeLoaders2, setter, nuxt = nuxtMock) {
...
function createComposableContext({
  i18n: _i18n,
  runtimeI18n,
  getDomainFromLocale
}) {
  ...
  function getRouteBaseName$1(route) {
    return getRouteBaseName(route, "___");
  }
  function resolveLocalizedRouteByName(route, locale) {
    route.name || (route.name = getRouteBaseName$1(router.currentRoute.value));
  ...
  function resolveLocalizedRouteByPath(input, locale) {
    ...
    const baseName = getRouteBaseName$1(route);
    if (baseName) {
      route.name = getLocalizedRouteName2(baseName, locale);
      return route;
    }
    if (prefixable(locale, runtimeI18n.defaultLocale)) {
      route.path = "/" + locale + route.path;
    }
    route.path = withoutTrailingSlash(route.path, true);
    return route;
  }
  return {
    router,
    ...
    getRouteBaseName: getRouteBaseName$1,
    getLocalizedDynamicParams: (locale) => {
      const params = router.currentRoute.value.meta["nuxtI18nInternal"] ?? {};
...
function createBaseUrlGetter(nuxt) {
...
function createLocaleRouteNameGetter(defaultLocale) {
  const localizeRouteName = (name, locale, isDefault) => getLocalizedRouteName(name, locale, isDefault, "___", "default");
...
const route_locale_detect__HPHJq3Jg7gwhwgKEI8tQavopSAjmrCSPXl9HgL2h9U = /* @__PURE__ */ defineNuxtPlugin({
  ...
  async setup() {
    ...
    [__temp, __restore] = executeAsync(() => handleRouteDetect(nuxt.$router.currentRoute.value)), await __temp, __restore();
    addRouteMiddleware(
      "locale-changing",
...
const markdown_it_meta_tags_W46zerXqlQbjovQYvEyCPRGi_qQbnfNPu7mA_tTGvJo = /* @__PURE__ */ defineNuxtPlugin(() => {
  const md = useMarkdownIt();
  ...
  md.core.ruler.push("parse_block_meta", (state) => {
    ...
    for (let i = 0; i < tokens.length; i++) {
      ...
      if (token.type === "paragraph_open" && ((_a = tokens[i + 1]) == null ? void 0 : _a.type) === "inline") {
...
const markdown_challenge_P0gGr31B925E6AYnvSNblWJBMmXRx962YYsht5kkdTs = /* @__PURE__ */ defineNuxtPlugin((nuxtApp) => {
  nuxtApp.hook("content:markdown", (md) => {
    md.use(container, "challenge", {
      render(tokens, idx) {
        var _a;
        const token = tokens[idx];
        if (token.nesting === 1) {
          const content = ((_a = tokens[idx + 1]) == null ? void 0 : _a.content) || "";
...
const markdown_explain_0QcZS_DyEYpLYHXBX9UsoAN9PzpP3CHjzuQBPQt7YOE = /* @__PURE__ */ defineNuxtPlugin((nuxtApp) => {
  nuxtApp.hook("content:markdown", (md) => {
    md.use(container, "explain", {
      render(tokens, idx) {
        const token = tokens[idx];
...
const og_meta_global_HbSHPjGAJP5wda2sXcISvSZkpK4uqhi58eobdRGE8G8 = /* @__PURE__ */ defineNuxtPlugin((nuxtApp) => {
...
const schema_global_NU33uF46HoO8pHeC8H6GYhO8UIupGQsEfZWNhk4_gX8 = /* @__PURE__ */ defineNuxtPlugin(() => {
  useHead({
    script: [
      {
...
const seo_global_8CYuz5uyPxNyKZxXILTAQPmrRKEHA7f695hTL7rWJR4 = /* @__PURE__ */ defineNuxtPlugin((nuxtApp) => {
  nuxtApp.hook("app:rendered", () => {
    const route = useRoute();
    const lang = route.params.lang || "en";
    const slug = route.params.slug;
...
const i18n_hydration_Mdw4wrM75Mn4G_NXfGNqvWWlUR0xD2LCsGgWv62hUOU = /* @__PURE__ */ defineNuxtPlugin({
  // Run this plugin after i18n is initialized
  enforce: "post",
  // (We'll explain enforce: 'post' below)
  setup(nuxtApp) {
    {
...
const plugins = [
  ...
  markdown_it_challenge_DXOI_Idopd0YDGYpvj7Hajb_j8G1bunaAWwPmIlz9nw,
  markdown_it_explain_bFnrxYtVO4fmzkuyY2_B2JY5UW0pG5rzvAe18MKP6A8,
  markdown_it_meta_tags_W46zerXqlQbjovQYvEyCPRGi_qQbnfNPu7mA_tTGvJo,
  markdown_challenge_P0gGr31B925E6AYnvSNblWJBMmXRx962YYsht5kkdTs,
  markdown_explain_0QcZS_DyEYpLYHXBX9UsoAN9PzpP3CHjzuQBPQt7YOE,
  og_meta_global_HbSHPjGAJP5wda2sXcISvSZkpK4uqhi58eobdRGE8G8,
  schema_global_NU33uF46HoO8pHeC8H6GYhO8UIupGQsEfZWNhk4_gX8,
  seo_global_8CYuz5uyPxNyKZxXILTAQPmrRKEHA7f695hTL7rWJR4,
  ssg_detect_IpHCGcQQ_IR5Rl99qyukWoMA9fJGfuTYyoksTzy81cs,
  i18n_hydration_Mdw4wrM75Mn4G_NXfGNqvWWlUR0xD2LCsGgWv62hUOU
];
...
function _sfc_ssrRender(_ctx, _push, _parent, _attrs) {
  _push(`<header${ssrRenderAttrs(mergeProps({ class: "p-4 bg-gray-100 dark:bg-gray-900" }, _attrs))}>Header</header>`);
}
...
const AppHeader = /* @__PURE__ */ _export_sfc(_sfc_main$4, [["ssrRender", _sfc_ssrRender]]);
const _sfc_main$3 = {
  __name: "AppFooter",
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<footer${ssrRenderAttrs(mergeProps({ class: "w-full border-t b-solid b-gray-200 dark:b-dark-700 bg-white dark:bg-black py-6 mt-8" }, _attrs))}><div class="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between px-4 space-y-4 md:space-y-0"><nav class="flex flex-wrap gap-4 text-sm">`);
      ...
      _push(ssrRenderComponent(_component_NuxtLink, {
        ...
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            ...
            return [
              createTextVNode(toDisplayString$1(unref(t)("nav.about")), 1)
            ];
          }
        }),
        _: 1
      ...
      _push(ssrRenderComponent(_component_NuxtLink, {
        ...
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            ...
            return [
              createTextVNode(toDisplayString$1(unref(t)("nav.contact")), 1)
            ];
          }
        }),
        _: 1
      ...
      _push(`</nav><div><select${ssrRenderAttr("value", unref(locale).value)} class="px2 py1 rounded text-sm b border-gray-300 dark:border-dark-700 bg-white dark:bg-dark-800 text-black dark:text-white"><!--[-->`);
      ssrRenderList(unref(locales), (loc) => {
        _push(`<option${ssrRenderAttr("value", loc.code)}>${ssrInterpolate(loc.name)}</option>`);
      });
      _push(`<!--]--></select></div><div class="text-xs text-gray-500 dark:text-gray-400 flex items-center gap-2"><span>© ${ssrInterpolate(unref(year))} SoCalSolver</span><span class="mx-2">|</span><a href="https://twitter.com/" target="_blank" rel="noopener" aria-label="Twitter" class="hover:underline">Twitter</a><a href="https://linkedin.com/" target="_blank" rel="noopener" aria-label="LinkedIn" class="hover:underline">LinkedIn</a></div></div></footer>`);
    };
  }
};
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(ssrRenderComponent(_component_ColorScheme, _attrs, {
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`<div class="flex flex-col min-h-screen bg-white dark:bg-black text-black dark:text-white transition-colors duration-300"${_scopeId}>`);
            _push2(ssrRenderComponent(AppHeader, null, null, _parent2, _scopeId));
            _push2(`<main class="flex-1 px4 py6 md:px8 md:py10"${_scopeId}>`);
            ...
            return [
              createVNode("div", { class: "flex flex-col min-h-screen bg-white dark:bg-black text-black dark:text-white transition-colors duration-300" }, [
                createVNode(AppHeader),
                createVNode("main", { class: "flex-1 px4 py6 md:px8 md:py10" }, [
